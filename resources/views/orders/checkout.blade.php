    ...
    <section class="page-shell">
        ...
        <form action="{{ route('order.place') }}" method="POST" data-no-swup class="mt-8 grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <div class="space-y-6">
                ...
                <div class="card-surface p-6 lg:p-8">
                    ...
                    <div class="mt-6 grid gap-4 md:grid-cols-2">
                        <input type="text" name="label" value="{{ old('label') }}" class="field mt-0" placeholder="Label (e.g. Home)">
                        <input type="text" name="full_name" value="{{ old('full_name', auth()->user()->name) }}" class="field mt-0" placeholder="Full name">
                        <input type="text" name="phone" value="{{ old('phone', auth()->user()->phone) }}" class="field mt-0" placeholder="Phone">
                        <input type="text" name="city" value="{{ old('city') }}" class="field mt-0" placeholder="City">
                        <input type="text" name="governorate" value="{{ old('governorate') }}" class="field mt-0" placeholder="Governorate">
                        <input type="text" name="country" value="{{ old('country', 'Egypt') }}" class="field mt-0" placeholder="Country">
                        <textarea name="street" rows="3" class="field mt-0 md:col-span-2" placeholder="Street address">{{ old('street') }}</textarea>
                    </div>
                </div>

                {{-- Payment --}}
                <div class="card-surface p-6 lg:p-8">
                    <h2 class="font-serif text-xl font-semibold text-bunny-text dark:text-bunny-dark-text">Payment</h2>
                    <div class="mt-5 grid gap-3">
                        @foreach(['cash_on_delivery' => 'Cash on delivery', 'credit_card' => 'Credit card', 'paymob' => 'Paymob', 'fawry' => 'Fawry'] as $value => $label)
                            <label class="flex cursor-pointer items-center gap-3 rounded-2xl border border-bunny-border p-4 transition-all has-[:checked]:border-bunny-primary has-[:checked]:shadow-soft dark:border-bunny-dark-border dark:has-[:checked]:border-bunny-primary">
                                <input type="radio" name="payment_method" value="{{ $value }}" class="text-bunny-primary focus:ring-bunny-primary/20" @checked(old('payment_method', 'cash_on_delivery') === $value)>
                                <span class="text-sm font-medium">{{ $label }}</span>
                            </label>
                        @endforeach
                    </div>
                    <textarea name="notes" rows="3" class="field" placeholder="Order notes (optional)">{{ old('notes') }}</textarea>
                </div>
            </div>
            ...
            <aside class="card-surface h-fit p-6 lg:p-8">
                ...
                <button type="submit" class="pill-btn-primary mt-6 w-full text-center">Place order</button>
            </aside>
        </form>
    </section>
@endsection
